...
if not config.disable_chats.local_chat then
    utils.commands.register('local', nil, 'Send a local chat message only visible to those in range.',
        { { name = 'message', help = 'The message to say' } }, function(source, args, raw)
...
if not config.disable_chats.staff then
    utils.commands.register('staff', { 'dev' }, 'Send a staff message.',
        { { name = 'message', help = 'The message to say' } }, function(source, args, raw)
        local message = table.concat(args, " ")
        local player_name = get_player_name(source)
        log_message({
...
if not config.disable_chats.staff_only then
    utils.commands.register('staff_only', { 'dev' }, 'Send a staff only message.',
        { { name = 'message', help = 'The message to say' } }, function(source, args, raw)
        local message = table.concat(args, " ")
        local player_name = get_player_name(source)
        log_message({
...
if not config.disable_chats.advert then
    utils.commands.register('ad', nil, 'Send a local chat message only visible to those in range.',
        { { name = 'message', help = 'The message to say' } }, function(source, args, raw)
        local message = table.concat(args, " ")
        local player_name = get_player_name(source)
        log_message({
...
if not config.disable_chats.police then
    utils.commands.register('police', nil, 'Send a police message to all players.',
        { { name = 'message', help = 'The message to say' } }, function(source, args, raw)
        local message = table.concat(args, " ")
        local player_name = get_player_name(source)
...
if not config.disable_chats.ems then
    utils.commands.register('ems', nil, 'Send a ems message to all players.',
        { { name = 'message', help = 'The message to say' } }, function(source, args, raw)
        local message = table.concat(args, " ")
        local player_name = get_player_name(source)
...
if not config.disable_chats.warning then
    utils.commands.register('warn', { 'dev' }, 'Send a warning to all players or specify a player.',
        { { name = 'id', help = 'Player ID (optional)' }, { name = 'message', help = 'The message to say' } },
...
if not config.disable_chats.pm then
    utils.commands.register('pm', nil, 'Send a private message to a specific player.',
        { { name = 'id', help = 'Player ID' }, { name = 'message', help = 'The message to say' } },
        function(source, args, raw)
            local target_id = tonumber(args[1])
            local player_name = get_player_name(source)
...
if not config.disable_chats.group then
    utils.commands.register('group', nil, 'Send a message to your group.',
        { { name = 'group_name', help = 'Name of the group' }, { name = 'message', help = 'The message to say' } },
        function(source, args, raw)
            if #args < 2 then
                TriggerClientEvent('chat:addMessage', source, {
                    template = [[
...
if not config.disable_chats.trade then
    utils.commands.register('trade', nil, 'Send a trade chat message to all players.',
        { { name = 'message', help = 'The message to say' } }, function(source, args, raw)
        local message = table.concat(args, " ")
        local player_name = get_player_name(source)
        log_message({
